...
def main(args):
    ...
    if len(args) != 1:
        sys.exit("Usage: python reward_exit_mechanism.py <graphml file>")
    ...
    for i in range(300):    
        ...
        for jjj in range(0,len(G)):
            ...
            if defender[jjj].get_addtax()>0:
                exit_mechanism.append(jjj)
            if defender[jjj].get_addtax()<=0 and jjj in exit_mechanism:
                exit_mechanism.remove(jjj)
